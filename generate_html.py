...
MODEL = "anthropic/claude-3.5-sonnet"  # Better for code generation
...
def get_code_from_llm(brief, checks, attachments):
    ...
    prompt = f"""You are an expert software engineer and code generator. Generate production-quality, fully functional code.
...
- **CRITICAL**: All generated code must be production-ready, fully functional, well-structured, properly styled, and include all necessary features (UI elements, error handling, validation).
- For web applications: Include complete HTML structure, modern CSS styling, functional JavaScript, proper layouts (grids/flexbox), input validation, and user feedback.
- For data tasks: Ensure valid JSON/CSV/YAML formatting, proper data structures, and error handling.
...
- For web apps: Include display screens, result areas, proper button layouts, clear/reset functionality, and modern styling.
